...
#pragma endregion
    ...
    DWORD C3DTransform::ColorAlphaBlend(DWORD dwDest, DWORD dwSrc)
    {
        unsigned int _alpha = dwSrc >> 24 ;
        if (0 == _alpha) return dwDest ;

        unsigned int rb = (((dwSrc & 0x00ff00ff) * _alpha) 
            + ((dwDest & 0x00ff00ff) * (0xff - _alpha))) & 0xff00ff00 ;
        unsigned int g = (((dwSrc & 0x0000ff00) * _alpha)
            +  ((dwDest & 0x0000ff00) * (0xff - _alpha))) & 0x00ff0000 ;

        return (dwSrc & 0xff000000) | ((rb | g) >> 8) ;
    }
